...
/*!
    \class Request
    \brief The base class for making requests to the cuteRadio Data API.
...
/*!
    ...
    \brief The access token used when making requests to the cuteRadio Data API.
*/
...
void Request::setAccessToken(const QString &token) {
    ...
    qDebug() << "CuteRadio::Request::setAccessToken" << token;
#endif
}

/*!
    \property QUrl Request::url
    \brief The url used when making requests to the cuteRadio Data API.
*/
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::Request::setUrl" << url;
#endif
}

/*!
    \property QVariantMap Request::headers
    \brief The headers used when making requests to the cuteRadio Data API.
*/
...
void Request::setHeaders(const QVariantMap &headers) {
    ...
    emit headersChanged();
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::Request::setHeaders" << headers;
#endif
}

/*!
    \property QVariant Request::data
    \brief The data used when making HTTP PUT/POST requests to the cuteRadio Data API.
*/
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::Request::setData" << data;
#endif
}
...
/*!
    ...
    when making requests to the cuteRadio API.
...
void Request::setNetworkAccessManager(QNetworkAccessManager *manager) {
    ...
    d->manager = manager;
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::Request::setNetworkAccessManager" << manager;
#endif
}
...
void Request::head(bool authRequired) {
    ...
    if (d->url.isEmpty()) {
        qDebug() << "CuteRadio::Request::head(): URL is empty";
        return;
    }
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::Request::head" << d->url;
#endif
...
void Request::get(bool authRequired) {
    ...
    if (d->url.isEmpty()) {
        qDebug() << "CuteRadio::Request::get(): URL is empty";
        return;
    }
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::Request::get" << d->url;
#endif
...
void Request::post(bool authRequired) {
    ...
    if (d->url.isEmpty()) {
        qDebug() << "CuteRadio::Request::post(): URL is empty";
        return;
    }
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::Request::post" << d->url << data;
#endif
    if (ok) {
        if (d->reply) {
            delete d->reply;
...
void Request::put(bool authRequired) {
    ...
    if (d->url.isEmpty()) {
        qDebug() << "CuteRadio::Request::put(): URL is empty";
        return;
    }
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::Request::put" << d->url << data;
#endif
    if (ok) {
        if (d->reply) {
            delete d->reply;
...
void Request::deleteResource(bool authRequired) {
    ...
    if (d->url.isEmpty()) {
        qDebug() << "CuteRadio::Request::deleteResource(): URL is empty";
        return;
    }
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::Request::deleteResource" << d->url;
#endif
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::RequestPrivate::setOperation " << Request::Operation(op);
#endif
}
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::RequestPrivate::setStatus " << Request::Status(s);
#endif
}
...
void RequestPrivate::setError(Request::Error e) {
    error = e;
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::RequestPrivate::setError " << Request::Error(e);
#endif
}
...
void RequestPrivate::setErrorString(const QString &es) {
    errorString = es;
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::RequestPrivate::setErrorString " << es;
#endif
}
...
void RequestPrivate::setResult(const QVariant &res) {
    result = res;
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::RequestPrivate::setResult " << res;
#endif
}
...
#ifdef CUTERADIO_DEBUG
    qDebug() << "CuteRadio::RequestPrivate::buildRequest " << u;
#endif
    QNetworkRequest request(u);
    
    if ((authRequired) && (!accessToken.isEmpty())) {
        request.setRawHeader("Authorization", "Basic " + QByteArray(accessToken.toUtf8() + ":").toBase64());
    }
